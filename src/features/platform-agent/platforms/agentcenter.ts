/**
 * AgentCenter platform adapter. MOCK: list/detail/publish all from mock data or mock success.
...
export const agentcenterAdapter: IPlatformAdapter = {
  ...
  async getAgentDetail(options: GetAgentDetailOptions): Promise<PlatformAgentApp | null> {
    if (options.id) return MOCK_AGENTCENTER_AGENTS.find((a) => a.id === options.id) ?? null
    if (options.name) {
      return (
        MOCK_AGENTCENTER_AGENTS.find(
          (a) =>
            a.name === options.name &&
  ...
  async invokeTool(options: InvokePlatformToolOptions): Promise<InvokePlatformToolResult> {
    return {
      success: true,
      output: `[AgentCenter mock] Invoked tool ${options.toolId} (${options.toolType}) for agent ${options.agentName}. Arguments: ${JSON.stringify(options.arguments ?? {})}`,
    }
  },
}
