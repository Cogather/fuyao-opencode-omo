...
function getConfigPaths(): { project: string; user: string; opencode: string } {
  ...
  return {
    project: join(cwd, ".opencode", "fuyao-opencode.json"),
    user: join(configDir, "fuyao-opencode.json"),
...
export function isServerInstalled(command: string[]): boolean {
  ...
  // Runtime wrappers (bun/node) are always available in fuyao-opencode context
  if (cmd === "bun" || cmd === "node") {
    return true
  }
