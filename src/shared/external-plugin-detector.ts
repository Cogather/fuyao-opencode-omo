/**
 * Detects external plugins that may conflict with fuyao-opencode features.
...
/**
 * Known notification plugins that conflict with fuyao-opencode's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[fuyao-opencode] External notification plugin detected: ${pluginName}

Both fuyao-opencode and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   fuyao-opencode's session-notification has been auto-disabled.

   To use fuyao-opencode's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in fuyao-opencode.json`
}
