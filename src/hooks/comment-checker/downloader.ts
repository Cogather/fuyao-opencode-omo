...
/**
 * Get the cache directory for fuyao-opencode binaries.
...
export function getCacheDir(): string {
  if (process.platform === "win32") {
    ...
    return join(base, "fuyao-opencode", "bin")
  }
...
export async function downloadCommentChecker(): Promise<string | null> {
  ...
  log(`[fuyao-opencode] Downloading comment-checker binary...`)
  ...
  try {
    ...
    log(`[fuyao-opencode] comment-checker binary ready.`)
    
    return binaryPath
    
  } catch (err) {
    debugLog(`Failed to download: ${err}`)
    log(`[fuyao-opencode] Failed to download comment-checker: ${err instanceof Error ? err.message : err}`)
    log(`[fuyao-opencode] Comment checking disabled.`)
    return null
  }
}
